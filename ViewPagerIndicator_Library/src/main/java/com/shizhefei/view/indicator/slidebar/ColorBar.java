...
public class ColorBar implements ScrollBar {
	...
	////////////////////////////////////////
	public ColorBar(Context context, int color, int height, int width) {
		this(context, color, height, width, Gravity.BOTTOM);
	}

	public ColorBar(Context context, int color, int height, int width, Gravity gravity) {
		view = new View(context);
		this.color = color;
		view.setBackgroundColor(color);
		this.height = height;
